...
def io_studio(device_id):
    dev=Device.query.filter_by(id=device_id,customer_id=current_user.customer_id).first_or_404();profile=studio_profile(dev);assets=Asset.query.filter_by(customer_id=current_user.customer_id).order_by(Asset.name).all()
    ...
    rows={x.channel_key:x for x in DeviceChannelAssignment.query.filter_by(device_id=dev.id).all()};return render_template('io_studio.html',device=dev,profile=profile,assets=assets,assignments=rows,is_mobile=profile['code']=='AT360_MOBILE_TRACKER')
...
def trends_limits(device_id):
    ...
    if request.method=='POST' and signal:
        ...
        signal.label=(request.form.get('label') or signal.label).strip()[:100];signal.unit=(request.form.get('unit') or signal.unit or '').strip()[:20];signal.raw_min=f('raw_min',signal.raw_min);signal.raw_max=f('raw_max',signal.raw_max);signal.eng_min=f('eng_min',signal.eng_min);signal.eng_max=f('eng_max',signal.eng_max);signal.warning_low=f('warning_low',signal.warning_low);signal.critical_low=f('critical_low',signal.critical_low);signal.warning_high=f('warning_high',signal.warning_high);signal.critical_high=f('critical_high',signal.critical_high);signal.deadband=max(0,f('deadband',signal.deadband));cfg=dict(signal.config_json or {});cfg.update({'sample_interval':request.form.get('sample_interval','5 seconds'),'upload_interval':request.form.get('upload_interval','1 minute'),'retention':request.form.get('retention','12 months'),'email_alarm':request.form.get('email_alarm')=='on','pulse_factor':request.form.get('pulse_factor'),'gear_ratio':request.form.get('gear_ratio'),'safe_startup_state':'OFF' if signal.signal_type=='OUTPUT' else cfg.get('safe_startup_state')});signal.config_json=cfg;db.session.commit();flash(f'{signal.label} saved.','ok')
