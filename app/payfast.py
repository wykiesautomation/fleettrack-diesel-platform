...
def valid_signature(form, cfg):
    ...
    # PayFast ITN implementations have used both received field order and
    # canonical key order. Validate only these two authenticated variants.
    received_pairs = list(form.items())
    received_signature = signature(received_pairs, cfg['passphrase'])

    canonical_pairs = sorted(
        ((str(key), str(value)) for key, value in form.items() if key != 'signature'),
        key=lambda pair: pair[0],
    )
    canonical_signature = signature(canonical_pairs, cfg['passphrase'])

    return supplied in {received_signature, canonical_signature}
