...
def test_data_cleanup():
    ...
    devices=Device.query.filter_by(customer_id=customer_id).order_by(Device.device_uid,Device.id).all()
    device_items=[{'device':device,'linked':bool(device.asset_id),'deletable':bool(not device.active and not device.asset_id)} for device in devices]
...
def analyse_tracking_points(rows):
    ...
    ordered=sorted(rows,key=lambda row:(aware(row.sampled_at),getattr(row,'id',0) or 0));strict=analyse_safety_twin_points(ordered);movement_by_time={}
    for point in strict.get('movement',[]):movement_by_time.setdefault(point['timestamp'],[]).append(point)
    rejected=[];rejection_counts={};fragments=[];current=[];previous_point=None;previous_time=None
    def close_segment():
        nonlocal current
        if current:fragments.append(current)
        current=[]
    def reject(row,stamp,reason):
        rejected.append({'timestamp':stamp,'latitude':row.latitude,'longitude':row.longitude,'accuracy':row.accuracy_m,'speed':row.speed_kmh,'reason':reason});rejection_counts[reason]=rejection_counts.get(reason,0)+1
    for row in ordered:
        ...
        if not point:
            accuracy=float(row.accuracy_m) if row.accuracy_m is not None else None;reject(row,stamp,'POOR_ACCURACY' if accuracy is not None and accuracy>50 else 'STATIONARY_DRIFT');close_segment();previous_point=None;previous_time=None;continue
        if previous_time is not None:
            gap_seconds=(row_time-previous_time).total_seconds()
            if gap_seconds<=0:reject(row,stamp,'OUT_OF_ORDER');close_segment();previous_point=None;previous_time=None;continue
            if gap_seconds>120:close_segment();previous_point=None;previous_time=None
            elif previous_point is not None:
                jump_km=_distance_dict(previous_point,point);implied_kmh=jump_km/(gap_seconds/3600.0)
                if jump_km>2.0 or implied_kmh>180.0:reject(row,stamp,'IMPLAUSIBLE_JUMP');close_segment();previous_point=None;previous_time=None;continue
    ...
    for fragment in fragments:
        if len(fragment)>=3:route_segments.append(fragment)
            ...
            for point in fragment:rejection_counts['INSUFFICIENT_ROUTE_EVIDENCE']=rejection_counts.get('INSUFFICIENT_ROUTE_EVIDENCE',0)+1;rejected.append({**point,'reason':'INSUFFICIENT_ROUTE_EVIDENCE'})
    distance=0.0;journeys=[]
    for number,segment in enumerate(route_segments,1):
        total=sum(_distance_dict(x,y) for x,y in zip(segment,segment[1:]));distance+=total;journeys.append({'number':number,'started_at':segment[0]['timestamp'],'ended_at':segment[-1]['timestamp'],'point_count':len(segment),'distance_km':round(total,3),'maximum_speed':round(max(float(x.get('speed') or 0) for x in segment),1)})
    stops=[{'timestamp':x.get('timestamp'),'latitude':x.get('latitude'),'longitude':x.get('longitude'),'accuracy':x.get('accuracy'),'reason':'STATIONARY_DRIFT'} for x in strict.get('drift',[])];accepted=[x for segment in route_segments for x in segment];context=strict.get('movement') or strict.get('drift') or [];last=accepted[-1] if accepted else (context[-1] if context else None);state=strict['state'] if accepted else ('WAITING_FOR_VALIDATED_ROUTE' if ordered else strict['state'])
...
def delete_device(device_id):
    ...
    if record.active or record.asset_id:
        flash('Disable and unlink the device before permanent deletion.','error')
        return redirect(url_for('main.test_data_cleanup'))
    expected_uid=str(record.device_uid or '').strip()
    if request.form.get('confirm_uid','').strip()!=expected_uid or request.form.get('confirm_word','').strip().upper()!='DELETE':
