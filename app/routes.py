...
def analyse_tracking_points(rows):
    ...
    return {'points':accepted,'rejected':rejected,'journeys':journey_rows,'stops':stops,'distance_km':round(distance,2),'max_speed':round(maximum),'last':accepted[-1] if accepted else None}
...
def tracking_history(asset_id):
    ...
    safety=(asset.metadata_json or {}).get('tracking_safety',{})
    zones=safety.get('zones',[]);speed_limit=float(safety.get('speed_limit_kmh') or 0);events=[]
    for point in analysis['points']:
        ...
        for zone in zones:
            try:
                center={'latitude':float(zone['latitude']),'longitude':float(zone['longitude'])};dist=_distance_dict(point,center)*1000;inside=dist<=float(zone.get('radius_m',250));breach=(zone.get('rule','KEEP_IN')=='KEEP_IN' and not inside) or (zone.get('rule')=='KEEP_OUT' and inside)
                if breach:events.append({'type':'GEOFENCE','severity':'CRITICAL','message':f"{zone.get('name','Zone')} {zone.get('rule','KEEP_IN').replace('_',' ').lower()} breach",'timestamp':point['timestamp'],'latitude':point['latitude'],'longitude':point['longitude']})
    ...
    motion_counts={key:sum(1 for x in motion_rows if x.event_type==key and x.status!='CANCELLED_BY_USER') for key in ('HARSH_BRAKING','SEVERE_BRAKING','HARSH_ACCELERATION','POSSIBLE_ACCIDENT','CRASH_DETECTED','ROLLOVER_DETECTED','EMERGENCY_ALERT')}
    motion_counts['CANCELLED']=sum(1 for x in motion_rows if x.status=='CANCELLED_BY_USER')
    driving_score=max(0,min(100,100-motion_counts['HARSH_BRAKING']*3-motion_counts['SEVERE_BRAKING']*6-motion_counts['HARSH_ACCELERATION']*2-motion_counts['POSSIBLE_ACCIDENT']*15-motion_counts['CRASH_DETECTED']*25-motion_counts['ROLLOVER_DETECTED']*35-motion_counts['EMERGENCY_ALERT']*10))

    # De-duplicate recurring events into operational episodes.
    episodes=[]
    for event in events:
    ...
    return render_template('tracking_history.html',asset=asset,start=start,end=end,analysis=analysis,safety=safety,zones=zones,safety_events=episodes[-100:],motion_events=motion_events,motion_counts=motion_counts,driving_score=driving_score,can_manage=current_user.role in ('customer_admin','platform_admin'))
...
def tracking_safety_save(asset_id):
    ...
    try:
        ...
        for zone in zones[:20]:
            clean.append({'name':str(zone.get('name') or 'Safety Zone')[:80],'latitude':max(-90,min(90,float(zone['latitude']))),'longitude':max(-180,min(180,float(zone['longitude']))),'radius_m':max(50,min(50000,float(zone.get('radius_m') or 250))),'rule':'KEEP_OUT' if zone.get('rule')=='KEEP_OUT' else 'KEEP_IN'})
    except (ValueError,TypeError,KeyError,json.JSONDecodeError):
        flash('Safety setup contains invalid zone data.','error');return redirect(url_for('main.tracking_history',asset_id=asset.id))
    meta=dict(asset.metadata_json or {});meta['tracking_safety']={'speed_limit_kmh':speed,'zones':clean,'updated_at':utcnow().isoformat(),'updated_by':current_user.id};asset.metadata_json=meta
    db.session.commit();flash('Tracking safety rules saved.','success');return redirect(url_for('main.tracking_history',asset_id=asset.id))
...
def asset_view(asset_id):
    ...
    return render_template('asset.html',asset=asset,signal_cards=cards,signal_lookup=lookup,chart_series=series,alarms=alarms,open_alarms=open_alarms,device=device,location=location,route_points=route,last_contact=last,generated_at=now,context=ctx,tank_stats=tank,tracking_stats=track,vibration_stats=vib,phone_battery=phone_battery,vehicle_summary=vehicle_summary,route_health=route_health,output_command=output_command,device_profile=device_profile,last_known_address=last_known_address,operational_battery=operational_battery,trend_policy=active_trend_policy,tank_orientation=((asset.metadata_json or {}).get('tank_visual') or {}).get('orientation','VERTICAL_CYLINDER'),monitoring_visual=(asset.metadata_json or {}).get('monitoring_visual','EASY_TANK' if asset.asset_type=='TANK' else 'GENERAL_MONITORING'),visual_signal_candidates=visual_signal_candidates,primary_visual_signal=primary_visual_signal,primary_visual_card=primary_visual_card,primary_trend_selected=primary_trend_selected,primary_trend_points=primary_trend_points)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            calibration.update({
                ...
                'orientation': request.form.get('orientation', 'VERTICAL_CYLINDER') if request.form.get('orientation') in ('VERTICAL_CYLINDER','HORIZONTAL_CYLINDER','RECTANGULAR','IRREGULAR') else 'VERTICAL_CYLINDER',
...
def mobile_tracker_config():
    ...
    return jsonify(status='ok',api_version='2026.2',device_uid=device.device_uid,asset_name=device.asset.name,
        ...
        features={'batch_upload':True,'offline_queue':True,'heartbeat':True,'battery':True,'charging':True,'speed':True,'heading':True,'safety_events':True})
...
def mobile_tracker_heartbeat():
    ...
    if str(data.get('device_id','')).upper()!=device.device_uid.upper():return jsonify(error='device_identity_mismatch'),403
    ensure_mobile_auto_profile(device)
    battery=data.get('battery_percent');sampled=utcnow()
    if battery is not None:
        try:battery=max(0,min(100,float(battery)))
        except (TypeError,ValueError):return jsonify(error='invalid_battery_percent'),400
        sig=SignalDefinition.query.filter_by(customer_id=device.customer_id,asset_id=device.asset_id,key='battery_percent').first()
        sequence=('heartbeat:'+str(data.get('sequence') or int(time.time())))[:80]
...
def mobile_tracker_location_batch():
    ...
    device.last_seen=utcnow();device.asset.last_seen=utcnow();db.session.commit()
    return jsonify(status='processed',accepted=accepted,duplicates=duplicates,rejected=rejected,accepted_count=len(accepted),duplicate_count=len(duplicates),rejected_count=len(rejected)),207 if rejected else 202
...
def mobile_tracking_start():
    ...
    consent=consent_for_device(device)
    if not consent or not consent.active:return jsonify(error='consent_inactive'),403
    consent.last_tracking_started_at=utcnow();device.last_seen=utcnow();audit(device.customer_id,'TRACKING_STARTED',device.asset_id,device.id,'DEVICE',None,'Tracking started by mobile API');db.session.commit()
...
def mobile_event():
    ...
    if event=='TRACKING_STARTED':
    ...
    elif event in ('HARSH_BRAKING','SEVERE_BRAKING','HARSH_ACCELERATION','POSSIBLE_ACCIDENT','POSSIBLE_ACCIDENT_CANCELLED','CRASH_DETECTED','ROLLOVER_DETECTED','EMERGENCY_ALERT'):
        if not consent or not consent.active:return jsonify(error='consent_inactive'),403
        ...
        severity='CRITICAL' if event in ('POSSIBLE_ACCIDENT','CRASH_DETECTED','ROLLOVER_DETECTED','EMERGENCY_ALERT') else 'HIGH' if event in ('HARSH_BRAKING','SEVERE_BRAKING') else 'WARNING'
        status='CANCELLED_BY_USER' if event=='POSSIBLE_ACCIDENT_CANCELLED' else 'POSSIBLE' if event=='POSSIBLE_ACCIDENT' else 'CONFIRMED' if event in ('CRASH_DETECTED','ROLLOVER_DETECTED','EMERGENCY_ALERT') else 'RECORDED'
        row=Live360SafetyEvent(customer_id=device.customer_id,asset_id=device.asset_id,device_id=device.id,event_type=event,severity=severity,confidence=confidence,status=status,sampled_at=sampled,latitude=lat,longitude=lon,accuracy_m=accuracy,speed_before_kmh=before,speed_after_kmh=after,peak_acceleration_ms2=peak,deceleration_ms2=decel,sequence=sequence,detail_json={'client_version':str(data.get('client_version') or '')[:40],'detection_version':'live360-safety-1.0'})
...
def ingest():
    ...
    if loc.get('latitude') is not None and not policy.gps_history_enabled:retain_latest_location_only(asset.id)
    device.last_seen=utcnow();asset.last_seen=utcnow();device.firmware=payload.get('firmware',device.firmware)
    db.session.commit();return jsonify(status='accepted',points=list(dict.fromkeys(stored+duplicates)),stored=list(dict.fromkeys(stored)),duplicates=list(dict.fromkeys(duplicates)),sequence=sequence),202
