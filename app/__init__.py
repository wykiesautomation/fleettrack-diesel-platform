    ...
    with schema_startup_lock(app):
        for attempt in range(3):
            try:
                ...
                from .device_io_migration import ensure_device_io_schema
                ensure_device_io_schema(db)
                db.create_all()
                return
...
def create_app(test_config=None):
    ...
    from .device_api import bp as device_api_bp
    ...
    with app.app_context():
        ...
        from .trend_cleanup import cleanup_due
        try:cleanup_due(force=False)
        except Exception:app.logger.exception("Startup trend cleanup failed safely")
        ...
        plan_specs = [
            ("essential", "Essential", 299.0, 1, ["Email alarms", "Optional 1- or 3-month device trends"]),
            ("monitor", "Monitor", 599.0, 1, ["Tracking or tank dashboard", "Optional 1- or 3-month device trends"]),
    ...
    @app.before_request
    def daily_trend_cleanup_fallback():
        if request.path.startswith('/static/'):
            return None
        try:
            from .trend_cleanup import cleanup_due
            cleanup_due(force=False)
        except Exception:
            db.session.rollback();app.logger.exception("Daily trend cleanup failed safely")
        return None
