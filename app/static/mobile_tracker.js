(() => {
  ...
  const HEARTBEAT_MS=60000,STALE_MS=150000,MAX_QUEUE=1000;
  let watchId=null,heartbeatId=null,healthId=null,lastPosition=null,batteryLevel=null,charging=null,uploading=false,serverConfig={heartbeat_interval_seconds:60,max_batch_points:100,max_offline_queue:1000};
  ...
  function showTracker(s){el('registerCard')?.classList.add('hidden');el('trackerCard')?.classList.remove('hidden');if(el('deviceUid'))el('deviceUid').textContent=s.device_uid||'-';if(el('assetName'))el('assetName').textContent=s.asset_name||'Mobile Tracker';updateNetwork();updateQueue();refreshStatus();}
  function showRegistration(){el('trackerCard')?.classList.add('hidden');el('registerCard')?.classList.remove('hidden');}
  ...
  function stopWatch(){if(watchId!==null)navigator.geolocation.clearWatch(watchId);watchId=null;if(heartbeatId)clearInterval(heartbeatId);heartbeatId=null;}
  function beginWatch(restored=false){
    ...
    watchId=navigator.geolocation.watchPosition(sendPosition,e=>log(`GPS error ${e.code}: ${e.message}`),{enableHighAccuracy:true,maximumAge:10000,timeout:30000});
    heartbeatId=setInterval(()=>{if(trackingWanted()&&lastPosition)uploadPayload(positionPayload(lastPosition,true));sendHeartbeat();},Math.max(30000,(serverConfig.heartbeat_interval_seconds||60)*1000));sendHeartbeat();
  ...
  document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'&&trackingWanted()&&watchId===null)beginWatch(true);});
  window.addEventListener('pageshow',()=>{if(trackingWanted()&&watchId===null)beginWatch(true);});
