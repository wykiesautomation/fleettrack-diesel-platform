(() => {
  ...
  window.cancelPossibleImpact=async()=>{if(!activeImpactEvent){log('No active possible-impact candidate to cancel.');return;}try{if(!activeImpactEvent.eventId)throw new Error('cancellation_id_unavailable');await requestJson(`/api/v1/mobile/events/${activeImpactEvent.eventId}/cancel`,{method:'POST',body:'{}'});if(impactTimer)clearInterval(impactTimer);impactTimer=null;activeImpactEvent=null;if(el('impactCountdown'))el('impactCountdown').classList.add('hidden');log('Possible impact cancelled by phone user.');}catch(error){log(`Impact cancellation failed: ${error.message}`);}};
